...
| 网址的上传图标（`/api/link_icon/<分组>/<网址>`） | ✅（私密模式未解锁时 403） | ✅ |
...
| 首页日历、到期提醒（浏览器里现算，只用上面已经公开的字段；已同步的放假安排随 `/api/configs` 下发） | ✅ | ✅ |
| 同步放假安排：去中国政府网检查 / 确认保存（`/api/holidays/check`、`/api/holidays/apply`） | ❌ | ✅ |
...
- 未解锁时 **所有** 接口都回 403，`/api/configs` 只回一个不含任何数据的空壳（`locked: true`，`chat: null`），
  页面据此只渲染时钟、搜索框和一张「收藏已锁定」卡片（点它去系统设置的解锁面板）——起始页被锁上时网页搜索照常能用，
  但分组导航、全局搜索、签到、聊天的入口都收起，地址栏手敲别的收藏页 / 签到页也只回到这张卡片；设置页里定时签到、
  自动刷新、代理、部署自检、配置恢复这几节也收起（空壳里的数值只是占位，不显示出来误导人）；
...
**3. 带着问题跳过去时指定模型 `HUB_CHAT_MODEL`**

- 只有「发送到 AI 聊天」生成的那条地址会多一个 `models=<模型>`（默认 `gpt-chat`）。不带问题地打开标签页
  不加这个参数，所以 HaloWebUI 的默认模型、新建对话、它自己的任何入口都不受影响。
- 值是整体百分号编码的，拆不散地址；进地址前先去掉空白与控制字符（对面的 `safeRedirectPath()`
  见到任何原样的控制字符会把整条 redirect 丢掉）。它不是凭据，进日志也无所谓。
...
- 管理会话 Cookie（`gyqd_session`，HttpOnly + SameSite=Lax，HTTPS 下带 Secure）有效期 30 天，**滑动续期**：
  已登录的浏览器打开首页（`GET /api/configs`）时，令牌签发超过一天就换一张新的 30 天令牌，于是只有 30 天没打开过
  才需要重新输入密码。签名密钥里含当前管理密码，改密码仍让所有旧令牌立即失效、也不会被续上；只用
  `X-Admin-Password` 请求头鉴权的调用不发 Cookie、不续期。
- 网址的上传图标不随 `/api/configs` 下发（体积大，起始页每开一次都要取），改由 `GET /api/link_icon/<分组 id>/<网址 id>`
  单独回吐：只按存储里已有的分组 / 网址 id 查找（请求里的字符串不进任何路径），内容就是保存时已按魔数校验过的
  PNG / JPEG / GIF / WebP，带 `nosniff`；地址里带内容哈希 `?v=`，对得上时 `Cache-Control: private, max-age=31536000, immutable`，
  对不上只给 `private, no-cache`。私密模式下它和其它接口一样要先解锁。导出 / 备份里仍是完整的 data URL。
- 图标底板的明暗判定完全在浏览器里完成：把已经加载好的同源图标（`/api/favicon`、`/api/link_icon` 或刚选好的 `data:` 上传图）缩到 24px
...
- 首页组件（日历 / 待办 / 倒数日 / 便签 / 到期提醒 / 签到状态）：倒数日与便签和待办同一个待遇——`/api/deck*` 全部要管理权限，
  ...
  日历、节日、农历全部在浏览器里现算（页面内置 1900–2100 年农历表与已知年份的放假安排），**浏览器不向任何外部服务发请求**，CSP 没有为它放宽；
...
- 日历的「同步放假安排」（管理员才看得到入口，两个接口都要管理权限）：由**服务端**去中国政府网取国务院办公厅的
  「关于 YYYY 年部分节假日安排的通知」，浏览器只和本站说话。出站请求只有两种地址——检索接口
  `https://sousuo.www.gov.cn/search-gov/data`（参数写死）和 `https://www.gov.cn/zhengce/(zhengceku|content)/…htm` 的通知正文；
  管理员粘贴的链接同样必须是后者（主机、端口、路径白名单，不带账号密码），跳转出白名单即失败，单次 12 秒超时、最多读 1MB，
  同一时间只跑一次、一分钟内重复点直接复用结果。它不是一个通用代理：任何别的地址在发请求之前就被拒绝。
  抓回来的 HTML 只当纯文本解析、从不下发给浏览器；解析结果逐条自检（括号里的星期几、「共 N 天」、七个节日齐全不重复、
  调休上班只在假期外的周末、元旦 / 劳动节 / 国庆节的假期分别含 1/1、5/1、10/1），任何一条不符就整份拒收。
  检查只预览；管理员点「保存」时凭预览的指纹写入 `data/holidays.json`（自带 `.bak`）——预览过期就按原条件重查，指纹不一致回 409、不写。
  读盘时同一套规则再验一遍，被改坏的年份直接丢掉（回落到页面内置的数据），来源链接不在白名单里的清空。放假安排是公开信息，
  所以随开放的 `/api/configs` 下发给访客（私密模式的空壳里没有）；页面渲染时节日名、文号一律转义，来源链接只认 `http(s)`。
...
| `HUB_CHAT_MODEL` | `gpt-chat` | 从本站**带着问题**跳过去时地址里指定的模型。只作用于那一条地址；光打开标签页不带，HaloWebUI 的默认模型不受影响。空 = 不指定 |
