...
- **收藏首页 = 浏览器起始页**：默认进入收藏首页，可直接设为浏览器主页（「系统设置 → 首页与起始页」有地址与各浏览器的设置步骤）。
  ...
  - **首页外观**（首页工具条「外观」或「系统设置 → 首页与起始页」）：只作用于收藏首页，其它页面保持工作台样式。
    ...
    - 有壁纸时首页固定为白字 + 遮罩（主题切换只影响其它页面）。遮罩分柔和 / 适中 / 加强三档；画面偏亮时会按「白字对比度 ≥ 4.5:1」自动加深，上传图片的亮度在压缩时顺手量好。
    - 半透明毛玻璃只用在搜索框和导航这两三块大面上；图标和小组件用半透明实底，手机不掉帧。浏览器不支持毛玻璃、系统开了「减少透明度」时换成实底；开了「提高对比度 / 强制颜色」时壁纸整个让路；省流量模式只显示主色渐变、不下载图片。
    - 电脑上首页的侧栏默认收成一条图标栏（可改回完整侧栏），进入分组、看板等页面自动恢复。
  - **站点图标清晰度**：抓取图标时高清优先（矢量、120–256px 位图、`apple-touch-icon`，站点没声明时还会探一次约定路径 `/apple-touch-icon.png`）；原图不到 64px 的小 favicon 不会被硬拉大，而是按原尺寸居中摆在底板上。个别站点抓不到高清图标时，可在编辑网址时上传备用图片。
