...
- **收藏首页 = 浏览器起始页**：默认进入收藏首页，可直接设为浏览器主页（「系统设置 → 首页与起始页」有地址与各浏览器的设置步骤）。
  ...
  - 两种密度：**图标**（默认，手机上四列、桌面上小分组自动并排，一屏看全）与**卡片**（显示描述、标签；看板站点显示全部监控字段并可就地刷新）。图标视图里，看板站点在名称下方带一条主指标（余额 / 到期倒计时），取数失败或即将到期会变色。
  - 看板里有站点取数失败 / 已过期 / 7 天内到期时，首页搜索框下方会出现提示，点一下直达看板并筛出这些站点。
  - 打开方式（新标签页 / 当前页）、搜索引擎、首页密度记在本浏览器的 Cookie 里，各设备可以各设各的。
