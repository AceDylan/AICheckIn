...
- **收藏首页 = 浏览器起始页**：默认进入收藏首页，可直接设为浏览器主页（「系统设置 → 首页与起始页」有地址与各浏览器的设置步骤）。
  ...
  - **外观**（首页右上角的「外观」或「系统设置 → 首页与起始页」）：壁纸与导航形态对**全部页面**生效——收藏首页、分组、站点看板、签到中心、系统设置是同一套样子，切页时不会在「壁纸起始页」和「工作台」之间来回跳。
    ...
    - 关掉壁纸时是「倒 L」版面：侧栏落在页面底色上，内容区是一块带细边的圆角画布；页头是标题 + 说明 + 操作 + 一条细分隔线，不再是一整块卡片。配色走「纸与墨」：界面本身是黑白的（中性色带一点暖，深色是暖炭黑、浅色是米白纸），主按钮是墨色实心（深色主题里白底黑字），唯一的彩色是信号蓝——焦点、链接、开关打开、今天；颜色留给站点图标、分组色和壁纸。按 OKLCH 取值，深 / 浅两套的次要文字在各自底色上都 ≥ 4.5:1。网址在分组页是一块面板里的一行行，看板卡片的字段不再各自套一个框；手机底栏浮成一条圆角胶囊。
